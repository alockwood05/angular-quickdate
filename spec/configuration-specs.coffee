    ...
    describe 'Given that the button icon html is configured', ->
      ...
      describe 'and given a datepicker where icon-class is set inline', ->
        beforeEach angular.mock.inject ($compile, $rootScope) ->
          ...
          element = $compile("<quick-date icon-class='icon-calendar' ng-model='myDate' />")($rootScope)
          scope.$digest()
    ...
    describe 'Given that it is configured without the timepicker', ->
      ...
      describe 'and given a datepicker with timepicker re-enabled', ->
        beforeEach(angular.mock.inject(($compile, $rootScope) ->
          ...
          element = $compile("<quick-date ng-model='myDate' disable-timepicker='false' />")(scope)
          $rootScope.$digest()
        ))
    ...
    describe 'Given that it is configured without a clear button', ->
      ...
      describe 'and given a datepicker with the clear button re-enabled', ->
        beforeEach(inject(($compile, $rootScope) ->
          scope = $rootScope
          scope.myDate = new Date()
          element = $compile("<quick-date ng-model='myDate' disable-clear-button='false' />")(scope)
          scope.$digest()
          console.log element
        ))
...
buildBasicDatepicker = ($compile, scope, date=new Date(), debug=false) ->
  scope.myDate = date
  if debug
    element = $compile("<quick-date debug='true' ng-model='myDate' />")(scope)
  else
    element = $compile("<quick-date ng-model='myDate' />")(scope)
  scope.$digest()
  element
